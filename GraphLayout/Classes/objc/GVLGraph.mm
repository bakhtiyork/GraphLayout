...
-(void)setAttribute:(NSString *)value forKey:(NSString *)key {
    agsafeset(self.node, (char*)[key UTF8String], (char*)[value UTF8String], empty);
}
...
-(void)prepare {
    ...
    CGPathRef cgpath = [GVLUtils toPathWithType:ND_shape(self.node)->name poly:(polygon_t*)ND_shape_info(self.node) width:width height:height];
    if (cgpath) {
...
-(void)setAttribute:(NSString *)value forKey:(NSString *)key {
    agsafeset(self.edge, (char*)[key UTF8String], (char*)[value UTF8String], empty);
}
...
-(void)prepare {
    ...
    CGAffineTransform move = CGAffineTransformMakeTranslation(-_frame.origin.x, -_frame.origin.y);
...
-(UIBezierPath*)toArrow:(const splines*)spl withHeight:(float)gheight {
    if((spl->list != 0) && (spl->list->size%3 == 1))
    {
        if(spl->list->eflag)
        {
            CGPoint p1 = [GVLUtils toPointF:spl->list->list[spl->list->size-1] height:gheight];
            CGPoint p2 = [GVLUtils toPointF:spl->list->ep height:gheight];
            return [GVLUtils arrowFrom:p1 to:p2 tailWidth:0.5 headWidth:8 headLength:12];
        }
    }
    return NULL;
}
...
-(void)setAttribute:(NSString*)value forKey:(NSString*)key {
    agsafeset(_graph, (char*)[key UTF8String], (char*)[value UTF8String], empty);
}
...
- (BOOL)loadLayout:(NSString *)text {
    ...
    _graph = graph;
    int i = 0;
    for (Agnode_t* ag_node = agfstnode(_graph); ag_node != NULL; ag_node = agnxtnode(_graph, ag_node))
    {
        GVLNode* node = [[GVLNode alloc] init];
        node.node = ag_node;
        node.parent = _graph;
        ...
        [node prepare];
        for (Agedge_t* ag_edge = agfstout(_graph, ag_node); ag_edge != NULL; ag_edge = agnxtout(_graph, ag_edge))
        {
            GVLEdge* edge = [[GVLEdge alloc] init];
            edge.edge = ag_edge;
            edge.parent = _graph;
...
+(NSMutableArray*)toPolygon:(const polygon_t *)poly width:(float)width height:(float)height {
    ...
    for (int side = 0; side < sides; side++) {
        CGPoint point = CGPointMake(vertices[side].x + width/2, vertices[side].y + height/2);
...
+(CGPathRef)toPathWithType:(const char *)type poly:(const polygon_t *)poly width:(float)width height:(float)height
{
    if ((strcmp(type, "rectangle") == 0) ||
        ...
        (strcmp(type, "Mdiamond") == 0) ||
        (strcmp(type, "Msquare") == 0) ||
        (strcmp(type, "star") == 0))
    {
        NSMutableArray* points = [GVLUtils toPolygon:poly width:width height:height];
...
+(void)setDpi:(float)value { _dpi = value; }
@end
